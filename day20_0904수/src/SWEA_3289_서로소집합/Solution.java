...
//				bF = findSet(b);
				...
				if(cal == 1) {
					...
					int aF = findSet(a);
					int bF = findSet(b);
					if(aF!= bF)	//대표가 동일한 경우(같은 집합에 속한 경우)
					union(aF, bF);	//대표가 상이할 경우 합침
				}
			}
	...
	static void union(int a, int b) {		//시간초과 났던 부분 : aF와 bF의 초기화와 비교를 union 안에서 한 것
			p[a] = b;	//a 대표자 요소에 b 대표자 값을 대입 -> a의 대표자는 b가 된다!
	}
